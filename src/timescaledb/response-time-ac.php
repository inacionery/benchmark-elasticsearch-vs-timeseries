...
Amp\Loop::run(function () {
    ...
    foreach ($hours as $i => $hour) {
        ...
        yield $connection->query("SELECT * FROM one_hour WHERE one_hour > '$from' AND one_hour <= '$to' ORDER BY visitors DESC LIMIT 20");
        yield $connection->query("SELECT * FROM one_hour_url WHERE one_hour > '$from' AND one_hour <= '$to'");
    }
